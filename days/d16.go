...
func (o operatorPacket) Eval() (result int) {
	...
	default:
		panic(fmt.Errorf("wrond type ID: %d", o.typeID))
	}
...
func NewBitstream(bs string) *bitstream {
...
func packetDecoderP1(s string) int {
	r, _ := NewBitstream(s).parse()
	return r.VersionSum()
}

func packetDecoderP2(s string) int {
	r, _ := NewBitstream(s).parse()
	return r.Eval()
}
